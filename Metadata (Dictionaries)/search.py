...
def title_to_info(metadata):
    info_dict = {}
    for article in metadata:
        info_dict[article[0]] = {'author': article[1], 'timestamp': article[2], 'length': article[3]}
    return info_dict
...
def search(keyword, keyword_to_titles):
    if keyword not in keyword_to_titles:
        return []
    title_list = keyword_to_titles[keyword]
    return title_list
...
def key_by_author(article_titles, title_to_info):
    ...
    for articles in article_titles:
        if title_to_info[articles]['author'] not in author_dict:
            author_dict[title_to_info[articles]['author']] = [articles]
        else: 
            author_dict[title_to_info[articles]['author']].append(articles)
    return author_dict
...
def filter_to_author(author, article_titles, title_to_info):
    article_list = []
    for articles in article_titles:
        if author == title_to_info[articles]['author']:
...
def articles_from_year(year, article_titles, title_to_info):
    ...
    unix_timestamp = (time.mktime(year_start.timetuple()), time.mktime(year_end.timetuple()))
    article_list = []
    for article in article_titles:
        if title_to_info[article]['timestamp'] >= unix_timestamp[0] and title_to_info[article]['timestamp'] <= unix_timestamp[1]:
            article_list.append(article)
    return article_list
